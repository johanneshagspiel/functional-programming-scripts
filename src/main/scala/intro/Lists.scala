...
object Lists {
    ...
    /** Q2 (3p)
      ...
      * Assume the list always has at least `n` items and that `n` is a positive integer.
      *
      ...
      * It contains some useful functions for this exercise.
      */

    def customAverage(xs: List[Int], x: Int, n: Int): Int = {

        def helperAverage (intList: List[Int], length: Int, sum: Int, numberToCompareTo: Int, numberElements: Int): Int = {
            intList match {
                case Nil => sum / length
                case x :: tail =>
                    if (numberElements == 0) sum / length
                    else if (x > numberToCompareTo) helperAverage (tail, length + 1, sum + x, numberToCompareTo, numberElements -1)
                    else helperAverage (tail, length, sum, numberToCompareTo, numberElements)
            }
        }

        helperAverage (xs, 0, 0, x ,n)
    }
}
